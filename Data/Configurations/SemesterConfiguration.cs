...
public class SemesterConfiguration : IEntityTypeConfiguration<Semester>
{
    public void Configure(EntityTypeBuilder<Semester> builder)
    {
        ...
        {
            table.HasCheckConstraint("CK_SEMESTERS_status", "[status] IN ('PLANNED', 'ACTIVE', 'COMPLETED')");
        ...
        builder.Property(x => x.AcademicYearId).HasColumnName("academic_year_id").HasColumnType("bigint").IsRequired();
        ...
        builder.HasIndex(x => x.AcademicYearId).HasDatabaseName("IX_SEMESTERS_academic_year_id");
        builder.HasIndex(x => new { x.IsDeleted, x.Status }).HasDatabaseName("IX_SEMESTERS_isDelete_status");
        builder.HasOne(x => x.AcademicYear).WithMany(x => x.Semesters).HasForeignKey(x => x.AcademicYearId).OnDelete(DeleteBehavior.NoAction);
    }
}
